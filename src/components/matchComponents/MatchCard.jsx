    ...
    <>
      {participantsData.length > 0 && mainPlayer && (
        <Link to={`/match/${matchId}/player/${playerId}`} className="card-link">
          <Card className={`flex w-full mt-2 ${mainPlayer.win ? 'border-[#0397AB] bg-blue-500/10' : 'border-red-00 bg-red-500/10'}`}>
            {/* SPELLS */}
            <div className='flex flex-col m-3 ml-5 justify-center space-y-1'>
            ...
            {/* CHAMP */}
            <div className='flex-col mb-3 mt-3 text-center'>
              <img className='w-14 h-14 mb-1 rounded-full border-2 border-[#32281E]'
                ...
                alt="ChampionPhoto"
              />
              <p className='text-sm Spiegel-Regular text-[#F0E6D2]'>{mainPlayer.championName}</p>
            </div>
            ...
            {/* ITENS */}
            <div className='flex flex-col justify-center gap-1 mr-6'>
